...
```{r multiple linear regression for resale prices, echo = FALSE}
lm_resale_prices <- lm(resale_price ~ flat_type2 + floor_area_sqm + remaining_lease_length + region + storeys + month_2, data = filtered_data) #define linear regression model

summary(lm_resale_prices) #Review the results

lm_resale_prices_2 <- lm(resale_price ~ floor_area_sqm + remaining_lease_length + region + storeys + month_2, data = filtered_data) #define linear regression model 2 without flat_type variable

summary(lm_resale_prices_2) #Review the results 2

lm_resale_prices_3 <- lm(resale_price ~ flat_type2 + floor_area_sqm + remaining_lease_length + region + storeys, data = filtered_data) #define linear regression model 3 without month of year

summary(lm_resale_prices_3) #Review the results 3
```
...
- Month of sale
  - As predicted, resale prices would decrease in the middle of the year (May to July), increase towards the end of the year and then fall again.

#### Visualising simple effects
```{r effect plot, echo = FALSE, fig.align = "center"}
glm_resale_prices <- glm(resale_price ~ flat_type2 + floor_area_sqm + remaining_lease_length + region + storeys + month_2, data = filtered_data,
            family = gaussian()) #create selected model using glm
